...
class _GroupChatPageState extends State<GroupChatPage> {
  ...
  void _initUser() {
    channel.sink.add(jsonEncode({
      ...
      "payload": {
        ...
        "avatar_url": widget.groupAvatar ?? ''
      },
      "ref": "${refCounter++}"
    }));
    _userAvatars[widget.userId] = widget.groupAvatar ?? '';
  }
  ...
  Future<void> _loadOldMessages() async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        // ------------ CẬP NHẬT PARTICIPANTS TỪ MESSAGES ------------
  ...
  void sendTyping() {
    ...
    channel.sink.add(jsonEncode({
      "topic": getTopic(),
      "event": "typing",
      "payload": {"username": widget.username},
      "ref": "${refCounter++}"
    }));
  ...
  void _onData(dynamic data) {
    try {
      ...
      if (event == "new_message") {
        ...
        if (avatar != null && avatar.isNotEmpty) _userAvatars[senderId] = avatar;
      ...
      if (event == "typing") typingUserNotifier.value = payload["username"];
      if (event == "stop_typing") typingUserNotifier.value = null;

      if (event == "user_joined") {
        final joinedUserId = int.tryParse(payload["user_id"].toString()) ?? 0;
        final username = payload["username"] ?? "Người dùng";
        final avatar = payload["avatar_url"];
        if (avatar != null && avatar.isNotEmpty) _userAvatars[joinedUserId] = avatar;
