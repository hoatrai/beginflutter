          ...
          child: Column(
            children: [
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    // 🔧 FIX: tiêu đề "Lời mời" là chuỗi ngắn cố định, không
                    // cần Expanded/Flexible — để plain như vậy sẽ không giành
                    // suất flex với cụm icon bên phải (trước đó Expanded(title)
                    // + Spacer + Flexible(cluster) bị chia đều 3 phần khiến
                    // cụm icon chỉ được 1/3 bề ngang rồi bị FittedBox ép nhỏ
                    // xíu dù còn dư chỗ).
                    ShaderMask(
                    ...
                    // 🔧 FIX: cụm nút bên phải (Kèo của tôi + Newsfeed +
                    // Thông báo + Chat) giờ nằm trong Expanded — được CẢ khoảng
                    // trống còn lại sau tiêu đề (không chia sẻ với Spacer nữa),
                    // căn phải bằng Align, và chỉ tự co nhỏ qua FittedBox nếu
                    // thật sự không đủ chỗ (màn hình cực hẹp / zoom rất cao).
                    Expanded(
                      child: Align(
                              ...
                              GestureDetector(
                                ...
                                child: Container(
                                  ...
                                  child: Row(
                                    mainAxisSize: MainAxisSize.min,
                                    children: [
                                      const Icon(Icons.local_bar, size: 16, color: Colors.white),
                                      const SizedBox(width: 4),
                                      // 🔧 FIX: Flexible + ellipsis — khi zoom cỡ chữ
                                      // trên màn hình hẹp, nhãn tự co lại thay vì đẩy
                                      // icon newsfeed/thông báo bên phải ra ngoài màn hình.
                                      Flexible(
                                        child: Text(
                                          "Kèo của tôi",
                                          overflow: TextOverflow.ellipsis,
                            ...
                            return GestureDetector(
                              ...
                              child: Container(
                                ...
                                child: _RunningBorderCard(
                                  ...
                                  glow: const Color(0xFFB71C1C),
                                  duration: const Duration(milliseconds: 4200),
                                  borderRadius: 24,
                                  child: _StatusGlowCard(
                                    // 🟠 Sap dien ra: giu hieu ung tho phong-xep cu.
                                    active: isSoon,
                                    colorLight: const Color(0xFFFFC107),
                                    colorStrong: const Color(0xFFFF9800),
                                    duration: const Duration(milliseconds: 2400),
                                    intensity: 0.55,
                                    // 🔧 FIX: AspectRatio(1.55) thuần túy khiến chiều
                                    // cao card = chiều rộng / 1.55 — trên máy màn hình
                                    // hẹp (độ phân giải thấp), card co lại quá thấp
                                    // (ví dụ ~196px) trong khi cột icon hành động bên
                                    // dưới (Positioned bottom: 66, cao ~158px) được đặt
                                    // theo pixel cố định, không co theo. Kết quả: icon
                                    // Chat bị đẩy tràn lên phía trên card, chồng lên
                                    // badge khoảng cách "xx km" ở góc trên. Dùng
                                    // LayoutBuilder để tự tính chiều cao theo tỉ lệ
                                    // NHƯNG không bao giờ thấp hơn 240 — đủ chỗ cho cả
                                    // badge trên cùng lẫn cột icon dưới cùng.
                                    child: LayoutBuilder(
                                      builder: (context, cardConstraints) {
                                        final cardWidth = cardConstraints.maxWidth;
                                        final ratioHeight = cardWidth / 1.55;
                                        final cardHeight =
                                        ratioHeight < 240 ? 240.0 : ratioHeight;
                                        return SizedBox(
                                          width: cardWidth,
                                          height: cardHeight,
                                          child: Stack(
...
// 🏃 Vien "chay" sang quanh khung card - dung cho trang thai "dang dien ra".
// Khac voi _StatusGlowCard (vien nhap nhay dong loat / tho phong-xep), o day
// chi 1 doan sang ngan chay lien tuc doc theo chu vi bo-tron-goc cua card,
// tao cam giac "dang live" ro hon la chi sang-toi deu.
class _RunningBorderCard extends StatefulWidget {
  final bool active;
  final Color color;
  final Color glow;
  final Duration duration;
  ...
  final double segmentFraction; // 0..1, ty le doan sang / tong chu vi
  final Widget child;
  ...
  const _RunningBorderCard({
    ...
    this.segmentFraction = 0.30,
  });

  @override
  State<_RunningBorderCard> createState() => _RunningBorderCardState();
}

class _RunningBorderCardState extends State<_RunningBorderCard>
    with TickerProviderStateMixin {
  late AnimationController _controller;
  ...
  @override
  void didUpdateWidget(covariant _RunningBorderCard oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.duration != oldWidget.duration) {
      _controller.dispose();
      _controller = AnimationController(vsync: this, duration: widget.duration);
...
class _RunningBorderPainter extends CustomPainter {
  final double progress; // 0..1, vi tri hien tai cua doan sang tren chu vi
  final Color color;
  final Color glow;
  final double radius;
  final double strokeWidth;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Vien mo lam nen, luon hien de thay ranh gioi card ngay ca luc doan
    // sang dang o phia doi dien.
    ...
    final segLen = total * segmentFraction;
    final start = total * progress;

    void drawSegment(double from, double len) {
      if (len <= 0) return;
      final seg = metric.extractPath(from, from + len);
    ...
    // Doan sang co the "vat" qua diem dau chu vi (wrap-around) khi progress
    // gan 1.0, nen tach lam 2 doan ve khi vuot qua tong chieu dai.
    if (start + segLen <= total) {
      drawSegment(start, segLen);
    } else {
      final firstLen = total - start;
      drawSegment(start, firstLen);
      drawSegment(0, segLen - firstLen);
    }
  }
