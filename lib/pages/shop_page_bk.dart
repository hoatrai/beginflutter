...
class _ShopPageState extends State<ShopPage> with WidgetsBindingObserver {
  ...
  /*void _autoScroll() async {
    while (mounted) {
      ...
      if (!_findingScrollController.hasClients) continue;
      if (isUserScrolling) continue;

      final maxScroll = _findingScrollController.position.maxScrollExtent;
      final current = _findingScrollController.offset;

      final next = current + 8.0; // 🔥 TỐC ĐỘ THỰC

      if (next >= maxScroll) {
        _findingScrollController.jumpTo(0);
      } else {
        _findingScrollController.animateTo(
          next,
          duration: const Duration(milliseconds: 30), // 🔥 mượt
          curve: Curves.linear,
        );
      }
    }
  ...
  Future<void> fetchInviteStatus(int productId) async {
    try {
     ...
     // debugPrint("🔵 Fetch invite status: $productId");
     ...
     // debugPrint("🟢 Saved inviteStatusMap[$productId]");
    } catch (e) {
  ...
  /*Future<void> _initFlow() async {
    ...
    await fetchProducts();
    await _loadMyUserId();
    await loadFindingStatus(); // ✅ chỉ gọi 1 lần
    fetchChatCount();
    connectSocket();
  }*/
  Future<void> _initFlow() async {
    try {
      ...
      if (hasPermission) {
        loc = await getMyLocation().timeout(
          const Duration(seconds: 8),
          onTimeout: () => {"lat": 10.7769, "lng": 106.7009}, // fallback HCM
        );
      } else {
        loc = {"lat": 10.7769, "lng": 106.7009}; // fallback nếu không có quyền
      }

      myLat = loc['lat'];
      myLng = loc['lng'];
    } catch (e) {
      debugPrint("❌ getMyLocation error: $e");
      myLat = 10.7769;
      myLng = 106.7009;
    }
  ...
  Future<void> findingKeoOn() async {
    try {
      ...
      if (lat != null && lng != null) {
        ...
        if (district?.isEmpty ?? true) {
          debugPrint("❌ Không lấy được quận");
          return;
        }
      }

      final url = Uri.parse(
        "${AppConfig.webDomain}/wp-json/custom/v1/finding-keo/on",
      );
...
// ✅ THÊM ĐOẠN NÀY NGAY ĐẦU HÀM
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed && isOpeningLocation) {
  ...
  void _openChat(String creatorId, String pubName, {String avatarUrl = ""}) async {
   ...
   // debugPrint("🟢 Mở chat với: $pubName ($creatorId), avatar=$avatarUrl");
  ...
  Future<void> fetchProducts() async {
    ...
    if (response.statusCode == 200) {
      ...
      for (var p in newProducts) {
        ...
        try {
          final lat = double.tryParse(
            productMap['meta']['lat']?.toString() ?? '',
          );

          final lng = double.tryParse(
            productMap['meta']['lng']?.toString() ?? '',
          );

          if (lat == null || lng == null) {
            continue; // không có tọa độ
          }

          if (myLat == null || myLng == null) {
            continue; // chưa có vị trí user
          }
      ...
      parsedProducts.sort((a, b) {
        final d1 = (a['distanceKm'] ?? 9999) as double;
        final d2 = (b['distanceKm'] ?? 9999) as double;
  ...
  Future<void> fetchCreatorsBulk(List<String> ids) async {
    ...
    for (final product in products) {
     ...
     // debugPrint("🧩 Product creator_id = $id");
    ...
    channel.stream.listen((message) {
      try {
        ...
        // Khi có update slots/participants
          ...
          //final updatedParticipants = payload['participants'] as List? ?? [];
...
// -------------------- Khi user join product --------------------
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          //product['participants'].add({'user_id': myUserId});
  ...
  Future<void> _deleteItem(BuildContext context, int productId) async {
    //debugPrint("🗑️ Đưa product id = $productId vào thùng rác");
        ...
        child: Row(
          children: [
            ...
            Switch(
              ...
              onChanged: (val) async {
                userChangedFinding = true; // 🔥 đánh dấu user đã thao tác
                if (val) {
                  _showPickTime(); // chọn time trước
                } else {
                  setState(() {
                    isFindingKeo = false;
                    findingOption = null;
  ...
  void _selectTime(String time) async {
    if (justReturnedFromSettings) {
      debugPrint("🚫 Bỏ qua selectTime do vừa bật GPS");
                ...
                child: loading && products.isEmpty
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                     ...
                     /* debugPrint(jsonEncode(product['participants']));
                      debugPrint("🔥 PRODUCT ID: ${product['id']}");
                      debugPrint("👥 PARTI: ${jsonEncode(product['participants'])}");*/
...
// 🆕 Mới tạo trong 6h
                      ...
                      final priceText = (parsedPrice == 0)
                          ? "Miễn phí"
                          : "${currencyFormat.format(parsedPrice)} VND";*/
                                  ...
                                  children: [
                                    ...
                                    Builder(
                                      builder: (context) {
                                        ...
                                        /*debugPrint(
                                            "PID=$productId "
                                                "MAP=${inviteStatusMap.containsKey(productId)} "
                                                "JOIN=${inviteStatusMap[productId]?.joinedCount}"
                                        );
                                        debugPrint(
                                          "SHOW PID=$productId joined=$joinedCount max=$maxPeople",
                                        );*/

                                        return Padding(
                                          padding: const EdgeInsets.only(top: 6),
                                          child: Text(
                                            /*"Đã tham gia: $joinedCount / $maxPeople",*/
